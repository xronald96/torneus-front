...
export default function Tournament({ data }: { data: any[] }) {
	return (
		<div className='flex p-10 flex-col'>
			...
			<div className='flex flex-wrap border-main-2'>
				{data.map((tournament) => {
					return (
						<Link
							href={`/tournament/${tournament._id}`}
							className='border-2 w-52 rounded-md p-5 no-underline font-poppins font-semibold text-main hover:text-main mr-5 mb-5'
							key={tournament.name}
						>
